...
const ActualMessage = ({ conversationId, conversationData}) => {
  ...
  useEffect(() => {
    ...
    if (conversationId) {
      axios({
        method: "get",
        url: `http://localhost:3001/message/messagesCombo/${conversationId}`,
      })
        .then((result) => {
  ...
  const messageSubmitHandler = (e) => {
    ...
    axios({
      method: "post",
      url: "http://localhost:3001/message/messages",
      data,
      headers: config,
    })
      .then((result) => {
  ...
  return (
    <>
      ...
      {/* message body start  */}
      <div className="w-[100%] h-[70%] bg-[#f7fafa] p-5 overflow-y-scroll" ref={chatRef}>
        {!isLoading ? (
